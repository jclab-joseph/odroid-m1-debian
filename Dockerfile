...
RUN debootstrap \
    --arch=arm64 \
    --include=bash-completion,vim,util-linux,fdisk,openssl,passwd,systemd,openssh-server,openssh-sftp-server,openssh-client,ifupdown2,isc-dhcp-client,gnupg2,initramfs-tools \
    bullseye \
    /work/rootfs \
    ${DEBIAN_MIRROR}
